        ...
        Begin Section
            ...
            Begin
                ...
                Begin ComboBox
                    ...
                    RowSource ="SELECT SpeciesGroups.GroupID, SpeciesGroups.GroupName FROM SpeciesGroups ORDER B"
                        "Y SpeciesGroups.GroupName;"
                    ColumnWidths ="0;1440"
                    ColumnGroup =6
                    GroupTable =1
...
Private Const strGenericError As String = _
    "The Species form encountered an error."

Private Sub cmdCancel_Click()
'Quit out of adding new species
...
'and enables merging two species
...
If MsgBox("Are you sure you want to delete this record?", _
    ...
    lngDeleteID = Me.SpeciesID
    'Check for IDs
    If DCount("*", "Detections", "SpeciesID=" & lngDeleteID) > 0 Then
        'Prompt to merge
        If MsgBox("Do you want to switch IDs for this species to a " & _
            "remaining species in the list?", vbYesNo) = vbYes Then
            'Open merge form
            DoCmd.OpenForm "MergeSpecies", , , "SpeciesID=" & _
            ...
            If DCount("*", "Detections", "SpeciesID=" & lngDeleteID) > 0 Then
                ...
                If MsgBox("You canceled the merge operation." & vbNewLine & _
                    vbNewLine & "Do you still want to delete this species " & _
        ...
        Else
            'Delete IDs
            DoCmd.RunSQL "DELETE * FROM Detections WHERE SpeciesID=" & _
                lngDeleteID
        End If
    End If
    'Delete shortcut and details for this species
    DoCmd.RunSQL "DELETE * FROM SpeciesShortcuts WHERE SpeciesID=" & _
        lngDeleteID
    DoCmd.RunSQL "DELETE DetailShortcuts.* FROM DetectionDetails " & _
        "INNER JOIN DetailShortcuts ON DetectionDetails.DetailID = " & _
        "DetailShortcuts.DetailID WHERE DetectionDetails.SpeciesID=" & _
        lngDeleteID
    DoCmd.RunSQL "DELETE * FROM DetectionDetails WHERE SpeciesID=" & _
        lngDeleteID
    'Find appropriate record to move to after delete
    Me.RecordsetClone.FindFirst "SpeciesID=" & lngDeleteID
    Me.RecordsetClone.MoveNext
    If Me.RecordsetClone.EOF Then
        Me.RecordsetClone.MovePrevious
        Me.RecordsetClone.MovePrevious
    End If
    lngNextID = Me.RecordsetClone!SpeciesID
    'Delete species
    DoCmd.RunSQL "DELETE * FROM Species WHERE SpeciesID=" & lngDeleteID
    'Scroll to appropriate record
    Me.Requery
    Me.RecordsetClone.FindFirst "SpeciesID=" & lngNextID
...
'Open the form for editing species groups

DoCmd.OpenForm "SpeciesGroups", , , , , acDialog
Me.GroupID.Requery
...
'Show star if common name is blank

On Error GoTo ErrHandler

Me.lblCNReqd.Visible = IsNull(Me.CommonName)
If IsNull(Me.ShortName) Then
    Me.ShortName = Me.CommonName
End If
Exit Sub
...
If Not Me.CommonName = Me.OpenArgs Then
...
If Len(Me.OpenArgs) > 0 Then
    ...
    Me.CommonName = Me.OpenArgs
    Me.ShortName = Me.OpenArgs
    Me.cmdCancel.Enabled = True
    Me.cmdDelete.Enabled = False
    MsgBox "The species you added appears at the bottom of the form." & vbNewLine & _
        "Please look over the list to make sure that species isn't" & vbNewLine & _
        "already represented, and add the Latin name if you can." & vbNewLine & _
        vbNewLine & "If you don't want to add this species, press cancel."
End If
...
'Check if MergeSpecies form is present, hide delete button if not
'No merge species form in Photo ID modules
On Error Resume Next
strTest = Application.CurrentProject.AllForms("MergeSpecies").Name
If Not strTest = "MergeSpecies" Then
    Me.GroupID.Visible = False
    Me.GroupID.Width = 0
    Me.GroupID.Left = Me.ShortName.Left + Me.ShortName.Width
    Me.cmdDelete.Visible = False
    Me.cmdDelete.Width = 0
    Me.cmdDelete.Left = Me.ShortName.Left + Me.ShortName.Width
    Me.cmdGroups.Visible = False
    Me.cmdGroups.Width = 0
    Me.cmdGroups.Left = Me.ShortName.Left + Me.ShortName.Width
End If

Me.cmdHelp.Left = Me.cmdDelete.Left + Me.cmdDelete.Width - _
    Me.cmdHelp.Width
Me.InsideWidth = Me.cmdDelete.Left + Me.cmdDelete.Width + 120
Me.cmdDone.Left = (Me.InsideWidth - Me.cmdDone.Width) / 2
Me.cmdCancel.Left = (Me.InsideWidth - Me.cmdCancel.Width) / 2
...
DoCmd.SetWarnings False
strSQL = "DELETE * FROM SpeciesShortcuts WHERE (((Shortcut) Is Null));"
DoCmd.RunSQL strSQL

'Check for duplicates
Set db = CurrentDb
Set rs = db.OpenRecordset("SELECT Count(SpeciesID) As Num, Shortcut " & _
    "FROM SpeciesShortcuts WHERE (((Shortcut) Is Not Null)) " & _
    "GROUP BY Shortcut ORDER BY Count(SpeciesID);")
If rs.RecordCount > 0 Then
    rs.MoveLast
    If rs!Num > 1 Then
        'Duplicates found, cancel closing the form
        MsgBox "Duplicate species shortcut keys found." & vbNewLine & _
