...
Begin Form
    ...
    Right =28545
    Bottom =15015
    Filter ="SpeciesID=103"
    RecSrcDt = Begin
        0x81e215bd6e4ee440
    End
    RecordSource ="Species"
    Caption ="MergeSpecies"
        ...
        Begin FormHeader
            ...
            Begin
                Begin Label
                    ...
                    Name ="Label2"
                    Caption ="Merge Photo ID Species"
        ...
        Begin Section
            ...
            Begin
                ...
                Begin TextBox
                    ...
                    TabIndex =2
                    ForeColor =2366701
                    Name ="txtSpeciesToKeepReqd"
                    ControlSource ="=IIf([cboSpeciesToKeep] Is Null,\"*\",\"\")"
...
Private Sub cboSpeciesToKeep_AfterUpdate()
'Enable merge button

If IsNull(Me.SpeciesID) Or IsNull(Me.cboSpeciesToKeep) Then
...
DoCmd.Close acForm, "MergeSpecies"
...
HelpMsg Me.Name, "Merging Species"
...
Set db = CurrentDb
'Update details
strSQL = "UPDATE DetectionDetails INNER JOIN Detections ON " & _
    "DetectionDetails.DetailID = Detections.DetailID SET " & _
    "Detections.DetailID = DLookUp('DetailID','DetectionDetails'," & _
    chr(34) & "DetailText='" & chr(34) & " & [DetailText] & " & _
    chr(34) & "' AND SpeciesID=" & Me.cboSpeciesToKeep & chr(34) & _
    ") WHERE (((Detections.SpeciesID)=" & Me.SpeciesID & "));"
db.Execute strSQL
'Mark affected photos for update
strSQL = "UPDATE Photos INNER JOIN Detections ON " & _
    "Photos.ImageID = Detections.ImageID SET " & _
    "Photos.NeedsUpdate = True WHERE " & _
    "(((Detections.SpeciesID)=" & Me.SpeciesID & _
    ") Or ((Detections.SpeciesID)=" & Me.cboSpeciesToKeep & "));"
db.Execute strSQL
'Switch existing detections to remaining species
strSQL = "UPDATE Detections SET Detections.SpeciesID = " & Me.cboSpeciesToKeep & _
    " WHERE (((Detections.SpeciesID)=" & Me.SpeciesID & "));"
db.Execute strSQL
'Display results
MsgBox db.RecordsAffected & " detections were converted."
'Run combine detections proc in case duplicates were created
CombineDetections
'Update flags for affected photos
UpdateFlags
...
'Cancel if not coming from Species form

On Error GoTo ErrHandler

If IsLoaded("Species") Then
    Cancel = False
Else
    Cancel = True
    MsgBox "This form can only be accessed from the Species List Form."
End If
